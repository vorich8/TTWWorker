...
```json
{
  ...
  "forceCloseRunningBrowserBeforeLaunch": true
}
```
...
- Для этого добавлен `forceCloseRunningBrowserBeforeLaunch: true` — перед запуском раннер закрывает запущенные процессы этого браузера, чтобы CDP поднялся корректно.
